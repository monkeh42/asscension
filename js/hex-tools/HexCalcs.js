...
function drawHexGrid(ctx)
{
	...
	{
		worldMap.Hexes[h].draw(ctx);
	}
	drawHomeCountry(ctx);
}

function drawSingleCountry(c, hovering, ctx) {
	for(var h in countries[c].hexes)
	{
		worldMap.GetHexById(countries[c].hexes[h]).draw(ctx, hovering);
	}
}

function drawHomeCountry(ctx) {
	for(var h in countries[player.homeCountryID].hexes)
	{
		worldMap.GetHexById(countries[player.homeCountryID].hexes[h]).draw(ctx, false, true);
	}
}
